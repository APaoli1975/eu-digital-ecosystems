...
```{r}
# Robust diff builder that doesn't crash when overlap is thin/absent
make_diff <- function(c1, c2, ind) {
  A <- dat_win %>%
    filter(indicator == ind, city == c1) %>%
    select(year, a = value)
  B <- dat_win %>%
    filter(indicator == ind, city == c2) %>%
    select(year, b = value)

  w <- full_join(A, B, by = "year") %>%
    mutate(diff = a - b)

  if (nrow(w) == 0) {
    # return an empty frame with expected columns
    return(tibble(year = integer(), diff = numeric(),
                  pair = character(), indicator = character()))
  }
  w$pair <- paste(c1, "−", c2)
  w$indicator <- ind
  w
...
if (nrow(diff_long) >= 2) {
  ...
  cat("Note: insufficient overlapping years to plot within-group differences.\n")
}
```
